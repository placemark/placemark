...
export function getOptionParser() {
  ...
  var targetOpt = {
    ...
    whereOpt2 = {
      describe:
        "filter polygon boundaries using a JS expression (with A and B)",
    },
    eachOpt2 = {
  ...
  */
  ...
  parser
    ...
    .option("anchor", {
      type: "numbers",
      describe:
        "center of rotation/scaling (default is center of selected shapes)",
    })
  ...
  parser
    ...
    .option("other", {
      describe:
        "default color for categorical scheme (defaults to no-data color)",
    })
    .option("nodata", {
    ...
    .example(
      "Define a sequential color scheme and use it to create a new field\n" +
  ...
  parser
    ...
    .example(
      "Dissolve all polygons in a feature layer into a single polygon\n" +
  ...
  parser
    ...
    .example(
      "Add two calculated data fields to a layer of U.S. counties\n" +
  ...
  parser
    ...
    .option("interval", {
      describe:
        "size of grid cells in degrees (options: 5 10 15 30 45, default is 10)",
      type: "number",
    })
    .option("polygon", {
  ...
  parser
    ...
    .example(
      "Join a csv table to a Shapefile (don't auto-convert FIPS column to numbers)\n" +
  ...
  parser
    ...
    .option("interval", {
      describe:
        "distance between interpolated points (meters or projected units)",
      type: "distance",
    })
    .option("name", nameOpt)
    .option("target", targetOpt)
  ...
  parser
    .command("rename-fields")
    .describe("rename data fields")
    .option("fields", {
      DEFAULT: true,
      type: "strings",
      describe:
        "list of replacements (comma-sep.), e.g. 'fips=STATE_FIPS,st=state'",
    })
    .option("target", targetOpt);

  parser
    .command("rename-layers")
  ...
  parser
    ...
    */
    .option("variable", {
      ...
      describe:
        "JS expr. assigning to one of: interval= percentage= resolution=",
      type: "flag",
    })
    .option("planar", {
  ...
  parser
    ...
    .option("expression", {
      DEFAULT: true,
      describe:
        "expression or field for grouping features and naming split layers",
    })
    .option("ids", {
  ...
  parser.command("external").option("module", {
    DEFAULT: true,
    describe: "name of Node module containing the command",
  });

  parser
    .command("filter-points")
  ...
  parser
    ...
    .example(
      "Calculate the total area of a polygon layer\n" +
  ...
  parser.command("colors").describe("print list of color scheme names");
  ...
  parser.command("quiet").describe("inhibit console messages");
