...
export function closeUndershoots(lyr, dataset, opts) {
  var maxGapLen = opts.gap_tolerance
    ? convertIntervalParam(opts.gap_tolerance, getDatasetCRS(dataset))
    : 0;
  ...
  var extensions = dangles.reduce(function (memo, dangle) {
    var candidates = index.findPointEnclosureCandidates(
      dangle.point,
      maxGapLen
    );
...
function findUndershootTarget(endpoint, candidates, arcs, maxGapLen) {
  ...
  candidates.forEach(function (candId) {
    ...
    if (
      hit &&
      hit.distance <= maxGapLen &&
      (!target || hit.distance < target.distance)
    ) {
      target = hit;
    }
  });
...
function chooseCloserPoint(p, a, b) {
  return geom.distance2D(p[0], p[1], a[0], a[1]) <
    geom.distance2D(p[0], p[1], b[0], b[1])
    ? a
    : b;
}
...
function getArcExtension(hit, arcId, arcs) {
  var v0 = arcs.getVertex(arcId, -1),
    ...
    s1 = hit.segment[0],
    s2 = hit.segment[1],
    endPtNew = geom.findClosestPointOnSeg(
      endPtOld[0],
      endPtOld[1],
      s1[0],
      s1[1],
