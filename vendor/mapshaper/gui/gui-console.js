...
export function Console(gui) {
  ...
  function onKeyDown(evt) {
    ...
    if (kc == 27) {
      ...
      (kc == 37 || kc == 39) &&
      (!typing || (typingInConsole && !inputText))
    ) {
      if (kc == 37) {
      ...
      if (kc == 13) {
        ...
        kc == 32 &&
        (!typing || (inputText === "" && typingInConsole))
      ) {
  ...
  function toHistory(str) {
    if (historyId > 0) {
      // if we're back in the history stack
      if (peekHistory() === "") {
  ...
  function submit(str) {
    ...
    if (cmd) {
      if (cmd == "clear") {
        ...
        message("Available layers:", internal.getFormattedLayerList(model));
  ...
  function printExamples() {
    ...
    printExample(
      "Delete one state from a national dataset",
      "$ filter 'STATE != \"Alaska\"'"
    );
    printExample(
      "Aggregate counties to states by dissolving shared edges",
      "$ dissolve 'STATE'"
    );
