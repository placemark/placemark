...
export function BoxTool(gui, ext, mouse, nav) {
  ...
  var infoBtn = new SimpleButton(popup.findChild(".info-btn")).on(
    "click",
    function () {
      if (coords.visible()) hideCoords();
      else showCoords();
    }
  );
  ...
  function pixToCoords(bbox) {
    ...
    return internal.getRoundedCoords(
      bbox2,
      internal.getBoundsPrecisionForDisplay(bbox2)
    );
  }
