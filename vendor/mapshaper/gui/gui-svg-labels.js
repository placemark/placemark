...
export function toggleTextAlign(textNode, rec) {
  ...
  var value =
    (curr == "middle" && "start") || (curr == "start" && "end") || "middle";
...
export function autoUpdateTextAnchor(textNode, rec, p) {
  ...
  var labelCenterX =
    rect.left - svg.getBoundingClientRect().left + rect.width / 2;
...
function updateTextAnchor(value, textNode, rec) {
  ...
  if (
    (curr == "middle" && value == "end") ||
    (curr == "start" && value == "middle")
  ) {
    xshift = width / 2;
  } else if (
    (curr == "middle" && value == "start") ||
    (curr == "end" && value == "middle")
  ) {
    xshift = -width / 2;
