...
export var SimplifyControl = function (gui) {
  ...
  // init settings menu
  new SimpleButton(menu.findChild(".submit-btn").addClass("default-btn")).on(
    "click",
    onSubmit
  );
  ...
  new SimpleButton(el.findChild(".simplify-settings-btn")).on(
    "click",
    function () {
      if (menu.visible()) {
        menu.hide();
  ...
  gui.addMode(
    ...
    turnOn,
    turnOff,
    gui.container.findChild(".simplify-btn")
  );
  ...
  function showMenu() {
    ...
    var opts = internal.getStandardSimplifyOpts(
      dataset,
      dataset.info && dataset.info.simplify
    );
    menu.findChild(".planar-opt-wrapper").node().style.display = showPlanarOpt
      ? "block"
      : "none";
  ...
  function getSimplifyOptions() {
    var method =
      menu.findChild("input[name=method]:checked").attr("value") || null;
    return {
      method: method,
      percentage: _value,
      no_repair: true,
  ...
  function getSimplifyOptionsAsString() {
    ...
    if (opts.method == "visvalingam" || opts.method == "dp")
      str += " " + opts.method;
