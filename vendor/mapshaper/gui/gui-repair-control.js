...
export function RepairControl(gui) {
  ...
  model.on("update", function (e) {
    var flags = e.flags;
    var intersectionsMayHaveChanged =
      flags.simplify ||
      flags.proj ||
      flags.arc_count ||
      flags.snap ||
  ...
  function updateSync() {
    ...
    if (arcs.getRetainedInterval() > 0) {
      ...
      if (!XX) {
        ...
        XX = dataset.info.intersections = internal.findSegmentIntersections(
          arcs,
          opts
        );
      }
  ...
  function showIntersections(XX, arcs) {
    ...
    if (n > 0) {
      ...
      pointLyr = {
        geometry_type: "point",
        shapes: [internal.getIntersectionPoints(XX)],
      };
      map.setIntersectionLayer(pointLyr, { layers: [pointLyr] });
      readout.html(
        utils.format(
          '<span class="icon"></span>%s line intersection%s <img class="close-btn" src="images/close.png">',
          n,
