...
function exportSymbolsForSVG(lyr, dataset, opts) {
  ...
  var features =
    geojson.features || geojson.geometries || (geojson.type ? [geojson] : []);
...
export function exportDataAttributesForSVG(records, fields) {
  ...
  return records.map(function (rec) {
    ...
    for (var i = 0; i < validFields.length; i++) {
      obj["data-" + validFields[i].toLowerCase()] = validDataAttributeValue(
        rec[validFields[i]]
      );
    }
    return obj;
...
export function getEmptyLayerForSVG(lyr, opts) {
  ...
  if (
    layerHasLabels(lyr) ||
    layerHasSvgSymbols(lyr) ||
    layerHasFurniture(lyr)
  ) {
