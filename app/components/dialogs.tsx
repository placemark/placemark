...
const ReauthDialog = dynamic<{
  onClose: () => void;
  resolve: dialogState.DialogStateReauth["resolve"];
}>(() => import("app/components/dialogs/reauth").then((r) => r.ReauthDialog), {
  loading: () => <Loading />,
});
...
const GistDialog = dynamic<{
  modal: dialogState.DialogStateGist;
  onClose: () => void;
}>(() => import("app/components/dialogs/gist").then((r) => r.GistDialog), {
  loading: () => <Loading />,
});
...
const RenameFolderDialog = dynamic<{
  onClose: () => void;
  modal: dialogState.DialogStateRenameFolder;
}>(
  () =>
    import("app/components/dialogs/rename_folder").then(
      (r) => r.RenameFolderDialog
    ),
  {
    loading: () => <Loading />,
  }
...
const RenameMapDialog = dynamic<{
  onClose: () => void;
  modal: dialogState.DialogStateRenameMap;
}>(
  () =>
    import("app/components/dialogs/rename_map").then((r) => r.RenameMapDialog),
  {
    loading: () => <Loading />,
  }
);
...
const APIDialog = dynamic<{
  onClose: () => void;
}>(() => import("app/components/dialogs/api").then((r) => r.APIDialog));

const QuickswitcherIndexDialog = dynamic<{
  onClose: () => void;
}>(
  () =>
    import("app/components/dialogs/quickswitcher_index").then(
      (r) => r.QuickswitcherIndexDialog
    ),
  {
    loading: () => <Loading />,
  }
...
export const Dialogs = memo(function Dialogs() {
  ...
  const content = match(dialog)
    ...
    .with({ type: "rename_map" }, (modal) => (
      <RenameMapDialog modal={modal} onClose={onClose} />
    ))
    .with({ type: "rename_folder" }, (modal) => (
      <RenameFolderDialog modal={modal} onClose={onClose} />
    ))
    .with({ type: "cheatsheet" }, () => <CheatsheetDialog />)
    .with({ type: "circle_types" }, () => <CircleTypesDialog />)
    .with({ type: "gist" }, (modal) => (
      <GistDialog modal={modal} onClose={onClose} />
    ))
    ...
    .with({ type: "reauth" }, (modal) => (
      <ReauthDialog resolve={modal.resolve} onClose={onClose} />
    ))
    .exhaustive();
