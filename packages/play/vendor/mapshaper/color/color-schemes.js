...
function initSchemes() {
  ...
  addSchemesFromD3(
    "categorical",
    "Category10,Accent,Dark2,Paired,Pastel1,Pastel2,Set1,Set2,Set3,Tableau10"
  );
  addSchemesFromD3(
    "sequential",
  ...
  addSchemesFromD3(
    "rainbow",
    "Cividis,CubehelixDefault,Rainbow,Warm,Cool,Sinebow,Turbo,Viridis,Magma,Inferno,Plasma"
  );
  addSchemesFromD3(
    "diverging",
    "BrBG,PRGn,PRGn,PiYG,PuOr,RdBu,RdGy,RdYlBu,RdYlGn,Spectral"
  );
  testLib(); // make sure these schemes are all available
  addCategoricalScheme(
    "Category20",
...
export function isColorSchemeName(name) {
  initSchemes();
  return (
    index.categorical.includes(name) ||
    index.sequential.includes(name) ||
    index.diverging.includes(name) ||
    index.rainbow.includes(name)
  );
}
...
export function getColorRamp(name, n, stops) {
  ...
  if (index.categorical.includes(name)) {
    stop(
      name,
      " is a categorical color scheme (expected a sequential color scheme)"
    );
  }
  if (ramps && ramps[n]) {
    ramp = ramps[n];
