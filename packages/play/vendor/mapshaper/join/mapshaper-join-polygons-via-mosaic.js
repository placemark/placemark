...
export function joinPolygonsViaMosaic(targetLyr, targetDataset, source, opts) {
  ...
  var joinFunction = getPolygonToPolygonFunction(
    targetLyr,
    sourceLyr,
    mosaicIndex,
    opts
  );
  var retn = joinTableToLayer(
    targetLyr,
    sourceLyr.data,
    joinFunction,
    joinOpts
  ...
  if (opts.interpolate) {
    if (opts.duplication)
      stop("duplication and interpolate options cannot be used together");
...
function interpolateFieldsByArea(destLyr, sourceLyr, mosaicIndex, opts) {
  var mosaicRecords = getOverlapDataByTile(
    destLyr,
    sourceLyr,
    mosaicIndex,
    opts
...
function getPolygonToPolygonFunction(targetLyr, srcLyr, mosaicIndex, opts) {
  var mergedToSourceIds = getIdConversionFunction(
    targetLyr.shapes.length,
    srcLyr.shapes.length
  );
