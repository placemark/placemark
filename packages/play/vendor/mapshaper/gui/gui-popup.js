...
export function Popup(gui, toNext, toPrev) {
  ...
  var prevLink = El("span")
    .addClass("popup-nav-arrow colored-text")
    .appendTo(nav)
    .text("◀");
  var navInfo = El("span").addClass("popup-nav-info").appendTo(nav);
  var nextLink = El("span")
    .addClass("popup-nav-arrow colored-text")
    .appendTo(nav)
    .text("▶");
  var refresh = null;
  ...
  self.show = function (id, ids, lyr, pinned) {
    ...
    refresh = function () {
      var rec =
        (table &&
          (editable ? table.getRecordAt(id) : table.getReadOnlyRecordAt(id))) ||
        {};
  ...
  function render(el, rec, table, editable) {
    ...
    if (rows > 0) {
      ...
      tableEl.on("copy", function (e) {
        ...
        if (pasted != cleaned && !window.clipboardData) {
          // ignore ie
          (e.clipboardData || e.originalEvent.clipboardData).setData(
            "text",
            cleaned
          );
      ...
      el.html(
        utils.format(
          '<div class="note">This %s is missing attribute data.</div>',
  ...
  function renderRow(table, rec, key, type, editable) {
    var rowHtml =
      '<td class="field-name">%s</td><td><span class="value">%s</span> </td>';
    var val = rec[key];
...
function getFieldType(val, key, table) {
  ...
  return (
    internal.getValueType(val) ||
    internal.getColumnType(key, table.getRecords())
  );
}
