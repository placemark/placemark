...
import mproj from "vendor/mproj";
...
export function WriteFilesProxy(gui) {
  ...
  internal.replaceWriteFiles(function (files, opts, done) {
    ...
    if (!utils.isArray(files) || files.length === 0) {
      ...
      saveBlobToDownloadFolder(
        files[0].filename,
        new Blob([files[0].content]),
        done
      );
    } else {
      filename =
        internal.getCommonFileBase(utils.pluck(files, "filename")) || "output";
...
internal.setProjectionLoader(function (opts, done) {
  var libs = internal.findProjLibs(
    [opts.init || "", opts.match || "", opts.crs || ""].join(" ")
  );
  // skip loaded libs
