...
cmd.filterIslands = function (lyr, dataset, optsArg) {
  ...
  if (!opts.min_area && !opts.min_vertices) {
    message(
      "Missing a criterion for filtering islands; use min-area or min-vertices"
    );
    return;
  }
  ...
  message(
    utils.format("Removed %'d island%s", removed, utils.pluralSuffix(removed))
  );
};
  ...
  var pathFilter = function (path, i, paths) {
    if (path.length == 1) {
      ...
      if (counts[absArcId(path[0])] === 1) {
        // and not part of a donut hole
        if (!ringTest || ringTest(path)) {
          // and it meets any filtering criteria
