...
import {
  joinPolylinesToPolygons,
  joinPolygonsToPolylines,
} from "../join/mapshaper-polyline-polygon-join";
...
import {
  joinTableToLayer,
  validateFieldNames,
} from "../join/mapshaper-join-tables";
import {
  joinPointsToPolygons,
  joinPolygonsToPoints,
} from "../join/mapshaper-point-polygon-join";
...
cmd.join = function (targetLyr, targetDataset, src, opts) {
  ...
  if (opts.keys) {
    ...
    if (srcType == "point" && targetType == "polygon") {
      ...
      retn = joinPointsToPoints(
        targetLyr,
        src.layer,
        getDatasetCRS(targetDataset),
        opts
      );
      ...
      stop(
        utils.format(
          "Unable to join %s geometry to %s geometry",
          srcType || "null",
          targetType || "null"
        )
      );
...
function validateJoinFieldType(field, type) {
  if (!type || type == "object") {
    stop(
      "[" +
        field +
        "] field has an unsupported data type. Expected string or number."
    );
  }
}
