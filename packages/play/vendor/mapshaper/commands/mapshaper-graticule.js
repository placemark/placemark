...
cmd.graticule = function (dataset, opts) {
  ...
  if (dataset && !isLatLngDataset(dataset)) {
    ...
    if (!dest)
      stop("Coordinate system is unknown, unable to create a graticule");
...
function createGraticule(P, outlined, opts) {
  ...
  if (![5, 10, 15, 30, 45].includes(interval))
  ...
  xx.forEach(function (x) {
    if (
      edgeMeridians &&
      (tooClose(x, edgeMeridians[0]) || tooClose(x, edgeMeridians[1]))
    ) {
      return;
    }
  ...
  function createMeridianPart(x, ymin, ymax) {
    ...
    meridians.push(
      graticuleFeature(coords, { type: "meridian", value: roundCoord(x) })
    );
  }
