...
cmd.simplify = function (dataset, opts) {
  ...
  if (opts.percentage || opts.percentage === 0) {
    ...
    arcs.setRetainedInterval(
      convertSimplifyInterval(opts.interval, dataset, opts)
    );
  } else if (opts.resolution) {
    arcs.setRetainedInterval(
      convertSimplifyResolution(opts.resolution, arcs, opts)
    );
...
export function calcSphericalInterval(xres, yres, bounds) {
  ...
  var width =
    geom.degreesToMeters(bounds.width()) *
    Math.cos(bounds.centerY() * geom.D2R);
