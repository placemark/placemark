...
import {
  polylineToPoint,
  polylineToMidpoints,
} from "../paths/mapshaper-polyline-to-point";
...
cmd.createPointLayer = function (srcLyr, dataset, opts) {
  ...
  if (nulls > 0) {
    message(
      utils.format("%,d of %,d points are null", nulls, destLyr.shapes.length)
    );
  }
  if (srcLyr.data) {
...
function interpolatedPointsFromVertices(lyr, dataset, opts) {
  ...
  var coords;
  if (interval > 0 === false)
    stop("Invalid interpolation interval:", opts.interval);
...
function pointsFromDataTable(data, opts) {
  ...
  if (
    !opts.x ||
    !opts.y ||
    !data.fieldExists(opts.x) ||
    !data.fieldExists(opts.y)
  ) {
