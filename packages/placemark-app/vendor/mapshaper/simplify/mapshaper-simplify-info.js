...
import {
  useSphericalSimplify,
  getSimplifyMethod,
} from "../commands/mapshaper-simplify";
...
export function printSimplifyInfo(arcs, opts) {
  ...
  lines.push(
    utils.format(
      "Method: %s (%s) %s",
      name,
      spherical ? "spherical" : "planar",
      method == "weighted_visvalingam"
        ? "(weighting=" + Visvalingam.getWeightCoefficient(opts) + ")"
        : ""
    )
  );
  lines.push(
    utils.format("Removed vertices: %,d", stats.removed + stats.collapsedRings)
  );
  lines.push(
    utils.format(
      "   %.1f% of %,d unique coordinate locations",
      pct1 * 100,
      stats.uniqueCount
    )
  );
  lines.push(
    utils.format(
      "   %.1f% of %,d filterable coordinate locations",
      pct2 * 100,
      stats.removableCount
    )
  );
  lines.push(
    utils.format(
      "Simplification threshold: %.4f %s",
      arcs.getRetainedInterval(),
      spherical ? "meters" : ""
    )
  );
  ...
  lines.push(
    utils.format("   Mean displacement: %.4f", stats.displacementMean)
  );
  lines.push(utils.format("   Max displacement: %.4f", stats.displacementMax));
  if (dq) {
    lines.push(
      utils.format("   Quartiles: %.2f, %.2f, %.2f", dq[0], dq[1], dq[2])
    );
  }
  ...
  if (aq) {
    lines.push(
      utils.format("   Quartiles: %.2f, %.2f, %.2f", aq[0], aq[1], aq[2])
    );
  }
