...
import {
  convertSourceName,
  findCommandSource,
} from "../dataset/mapshaper-source-utils";
...
export function runCommand(command, catalog, cb) {
  ...
  try {
    // catch errors from synchronous functions
    ...
    if (name == "rename-layers") {
      ...
      targets = catalog.findCommandTargets(
        opts.target || (opts.combine_layers && "*")
      );
    } else if (
      name == "rotate" ||
      name == "info" ||
      ...
      if (targets.length == 1) {
        ...
        stop(
          "This command does not support targetting layers from different datasets"
        );
      }
    }
    ...
    if (opts.source) {
      source = findCommandSource(
        convertSourceName(opts.source, targets),
        catalog,
        opts
      );
    }
    ...
    if (name == "affine") {
      ...
      outputLayers = applyCommandToEachLayer(
        cmd.alphaShapes,
        targetLayers,
        targetDataset,
        opts
      ...
      outputLayers = cmd.mergeAndFlattenLayers(
        targetLayers,
        targetDataset,
        opts
      );
    ...
    if (outputDataset) {
      ...
      outputLayers &&
      targetDataset &&
      outputLayers != targetDataset.layers
    ) {
      ...
      if (opts.apart) {
        catalog.setDefaultTargets(
          splitApartLayers(targetDataset, outputLayers).map(function (dataset) {
            return {
              dataset: dataset,
...
function applyCommandToEachLayer(func, targetLayers) {
  ...
  return targetLayers.reduce(function (memo, lyr) {
    ...
    if (utils.isArray(result)) {
      // some commands return an array of layers
      memo = memo.concat(result);
    } else if (result) {
      // assuming result is a layer
      memo.push(result);
    }
    return memo;
  }, []);
