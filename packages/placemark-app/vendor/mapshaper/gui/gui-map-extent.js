...
export function MapExtent(_position) {
  var _scale = 1,
    _cx,
    _cy, // center in geographic units
    _contentBounds,
    _self = this,
    _frame;
  ...
  this.setBounds = function (b) {
    ...
    if (prev) {
      _scale =
        (_scale * fillOut(_contentBounds).width()) / fillOut(prev).width();
    } else {
      _cx = b.centerX();
      _cy = b.centerY();
    }
  ...
  function maxScale() {
    ...
    var xscale =
      _contentBounds.width() / _position.width() / xmax / minPixelScale;
    var yscale =
      _contentBounds.height() / _position.height() / ymax / minPixelScale;
