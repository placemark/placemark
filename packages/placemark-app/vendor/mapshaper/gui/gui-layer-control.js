...
export function LayerControl(gui) {
  var map = gui.map;
  var model = gui.model;
  var el = gui.container
    .findChild(".layer-control")
  ...
  function getLayerOrderSlug() {
    return sortLayersForMenuDisplay(model.getLayers())
      .map(function (o) {
  ...
  function turnOn() {
    isOpen = true;
    el.findChild("div.info-box-scrolled").css(
      "max-height",
      El("body").height() - 80
    );
    render();
  ...
  function renderLayer(lyr, dataset, opts) {
    ...
    html += rowHTML(
      "name",
      '<span class="layer-name colored-text dot-underline">' +
        formatLayerNameForDisplay(lyr.name) +
        "</span>",
      "row1"
    );
    if (opts.show_source) {
    ...
    if (opts.pinnable) {
      html +=
        '<img class="pin-btn unpinned" draggable="false" src="images/eye.png">';
      html +=
        '<img class="pin-btn pinned" draggable="false" src="images/eye2.png">';
    }
    html += "</div>";
    return html;
  }
  ...
  function initLayerDragging(entry, id) {
    ...
    entry.on("mousemove", function (e) {
      ...
      insertionClass =
        e.pageY - rect.top < rect.height / 2 ? "insert-above" : "insert-below";
  ...
  function initMouseEvents2(entry, id, pinnable) {
    ...
    // init name editor
    new ClickText2(entry.findChild(".layer-name")).on("change", function (e) {
      var target = findLayerById(id);
  ...
  function rowHTML(c1, c2, cname) {
    return utils.format(
      '<div class="row%s"><div class="col1">%s</div>' +
