...
export function InteractiveSelection(gui, ext, mouse) {
  ...
  gui.keyboard.on(
    ...
    !!"capture"
  ); // preempt the layer control's arrow key handler
  ...
  function draggable() {
    return (
      ...
      interactionMode == "location" ||
      interactionMode == "labels"
    );
  }
  ...
  function targetIsRollover(target) {
    while (target.parentNode && target != target.parentNode) {
      if (
        target.className &&
        String(target.className).indexOf("rollover") > -1
      ) {
        return true;
      }
  ...
  function eventIsEnabled(type) {
    ...
    if (
      (type == "drag" || type == "dragstart" || type == "dragend") &&
      !draggable()
    ) {
      return false;
    }
