...
export function MshpMap(gui) {
  ...
  this.setIntersectionLayer = function (lyr, dataset) {
    ...
    if (lyr) {
      ...
      _intersectionLyr.style = MapStyle.getIntersectionStyle(
        _intersectionLyr.layer
      );
    } else {
      _intersectionLyr = null;
    }
  ...
  this.getDisplayCRS = function () {
    ...
    if (_activeLyr && _activeLyr.geographic) {
      crs =
        _activeLyr.dynamic_crs ||
        internal.getDatasetCRS(_activeLyr.source.dataset);
    }
    return crs || null;
  };
  ...
  function onUpdate(e) {
    ...
    if (e.flags.simplify_method) {
      // no redraw needed
      return false;
    }

    if (e.flags.simplify_amount || e.flags.redraw_only) {
      // only redraw (slider drag)
      drawLayers();
      return;
    }
  ...
  function updateLayerStyles(layers) {
    layers.forEach(function (mapLayer, i) {
      if (mapLayer.active) {
        ...
        if (
          mapLayer.style.type != "styled" &&
          layers.length > 1 &&
          mapLayer.style.strokeColors
        ) {
