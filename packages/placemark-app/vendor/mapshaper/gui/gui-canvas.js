...
export function drawOutlineLayerToCanvas(obj, canv, ext) {
  ...
  var darkStyle = {
      ...
      strokeColor: style.strokeColors[1],
    },
    lightStyle = {
      strokeWidth: style.strokeWidth,
      strokeColor: style.strokeColors[0],
    };
  var filter;
...
function getArcFilter(arcs, ext, usedFlag, arcCounts) {
  ...
  return function (i) {
    var visible = true;
    if (usedFlag != arcCounts[i] > 0) {
      // show either used or unused arcs
      visible = false;
...
function getLineScale(ext) {
  ...
  if (mapScale < 0.5) {
    ...
    if (!internal.getStateVar("DEBUG"))
      // thin lines for debugging
      s *= Math.pow(mapScale - 99, 0.1);
  }
  return s;
}
